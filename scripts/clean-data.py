...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 clean-data.py <data_dir> <scale> <partition>")
        exit(1)
    data_dir = sys.argv[1]
    scale = sys.argv[2]
    partition = sys.argv[3]
    data_base_directory = f"{data_dir}/scale={scale}/partition={partition}/tbl"
    data_output_directory = f"{data_dir}/scale={scale}/partition={partition}/cleaned-tbl"
